...
exports.hellowWorld = onRequest((req, res) => {
    console.info("helloo world test log")
...
exports.getFlashSaleProducts = onRequest(async (req, res) => {
    ...
    try {
        ...
        const offersSnapshot = await admin.firestore().collection('product_offers').where('countries_offers', 'array-contains-any', [countryID]).where('product_id', 'in', ids).get();
